...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, coins REAL DEFAULT 100, GGs INTEGER DEFAULT 0, lost_coins REAL DEFAULT 0, won_coins REAL DEFAULT 0, status INTEGER DEFAULT 0, checks TEXT DEFAULT '[]')''')
    ...
    if len(args) > 1 and args[1].startswith("ref_"):
        try:
            ...
            if referrer_id != user_id:
                ...
                if not existing:
                    conn.execute("UPDATE users SET coins = coins + 50 WHERE id = ?", (str(referrer_id),))
                    conn.execute("UPDATE users SET coins = coins + 25 WHERE id = ?", (str(user_id),))
                    conn.execute("INSERT INTO referrals (referrer_id, referred_id, reward_amount, created_ts) VALUES (?, ?, ?, ?)",
                                 (str(referrer_id), str(user_id), 50, now_ts()))
                    conn.commit()
                    await message.answer(f"🎉 По реферальной ссылке! +25 {CURRENCY_NAME} вам!")
                conn.close()
        except:
            pass
    
    await state.clear()
    await message.answer(f"🎮 Добро пожаловать в <b>{CURRENCY_NAME}</b>!\n\nСтартовый баланс: {fmt_money(START_BALANCE)}", reply_markup=main_menu_kb())
...
async def menu_ref_cb(query: CallbackQuery, bot: Bot):
    ...
    conn.close()
    await query.message.edit_text(f"👥 <b>Рефералы</b>\n\nВаша ссылка:\n<code>{link}</code>\n\nПриглашено: {count}\nЗа каждого: +50 вам, +25 другу", reply_markup=main_menu_kb())
    await query.answer()
...
async def transfer_command(message: Message):
    ...
    if len(parts) < 3:
        return await message.answer("Формат: `пер @username 100` или `пер 123456789 100` или ответ на сообщение", parse_mode="HTML")
...
# ==================== БАЛАНС И БОНУС ====================
@dp.message(lambda m: normalize_text(m.text) in {"б", "баланс", "/balance"})
...
@dp.message(lambda m: normalize_text(m.text) in {"профиль", "/profile"})
async def profile_command(message: Message):
    ...
    conn.close()
    wr = (bets["wins"] / bets["total"] * 100) if bets["total"] > 0 else 0
    await message.answer(f"👤 <b>Профиль</b>\n\nБаланс: {fmt_money(float(user['coins']))}\nСтавок: {bets['total']}\nПобед: {bets['wins']}\nWinrate: {wr:.1f}%")

@dp.message(lambda m: normalize_text(m.text) in {"бонус", "/bonus"})
async def bonus_command(message: Message):
    ...
    if ok:
        ...
        conn.commit()
        await message.answer(f"🎁 +{fmt_money(reward)}\nБаланс: {fmt_money(bal)}")
    conn.close()

# ==================== ТОП ====================
@dp.message(lambda m: normalize_text(m.text) in {"топ", "/top"})
...
# ==================== ИГРЫ МЕНЮ ====================
@dp.message(lambda m: normalize_text(m.text) in {"игры", "/games"})
...
async def game_pick_cb(query: CallbackQuery):
    ...
    await query.message.answer(f"Пример: <code>{usage.get(game, '')}</code>", parse_mode="HTML")
    await query.answer()
...
def roulette_roll(choice: str) -> tuple[bool, float, str]:
    ...
    if choice in ["red", "black"] and ((choice == "red" and color == "red") or (choice == "black" and color == "black")):
        ...
        win, mult = True, 36.0
    return win, mult, f"Выпало {number}"
...
async def roulette_amount(message: Message, state: FSMContext):
    ...
    await message.answer("Выберите:", reply_markup=roulette_choice_kb())
...
async def crash_amount(message: Message, state: FSMContext):
    ...
    await message.answer("Введите множитель (1.1-10)")
...
async def cube_amount(message: Message, state: FSMContext):
    ...
    await message.answer("Угадайте число 1-6")
...
async def cube_guess(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer("Введите число 1-6")
...
async def tower_pick(query: CallbackQuery):
    ...
    if game["level"] >= len(TOWER_MULTIPLIERS):
        ...
        TOWER_GAMES.pop(user_id)
        await query.message.edit_text(f"🏆 Победа!\nВыплата: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
        return await query.answer()
...
async def tower_cancel(query: CallbackQuery):
    ...
    TOWER_GAMES.pop(user_id)
    await query.message.edit_text(f"❌ Возврат: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
    await query.answer()
...
async def gold_cash(query: CallbackQuery):
    ...
    if not game or game["step"] == 0:
        return await query.answer("Сделайте ход", show_alert=True)
...
async def mines_amount(message: Message, state: FSMContext):
    ...
    await message.answer("Сколько мин? (1-5)")
...
async def mines_count(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.answer("Введите число 1-5")
    if mines < 1 or mines > 5:
        return await message.answer("1-5 мин")
    ok, _ = reserve_bet(message.from_user.id, bet)
    await state.clear()
    if not ok:
        return await message.answer("Недостаточно средств")
...
async def mines_cell(query: CallbackQuery):
    ...
    if len(game["opened"]) >= safe_total:
        ...
        MINES_GAMES.pop(user_id)
        await query.message.edit_text(f"🏆 Все клетки открыты!\nВыплата: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
        return await query.answer()
...
async def mines_cash(query: CallbackQuery):
    ...
    if not game or len(game["opened"]) == 0:
        return await query.answer("Откройте клетку", show_alert=True)
    mult = mines_multiplier(len(game["opened"]), game["mines_count"])
    payout = game["bet"] * mult
...
async def mines_cancel(query: CallbackQuery):
    ...
    if not game:
        return await query.answer("Нет игры", show_alert=True)
    payout = game["bet"] if len(game["opened"]) == 0 else 0
    bal = finalize_reserved_bet(user_id, game["bet"], payout, "mines", "cancel")
    MINES_GAMES.pop(user_id)
    await query.message.edit_text(f"❌ Возврат: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
    await query.answer()# ==================== ОЧКО (BLACKJACK) ====================
def make_deck() -> list:
...
def ochko_text(game: Dict[str, Any], reveal_dealer: bool) -> str:
    ...
    if reveal_dealer:
        ...
        dealer_line = f"{game['dealer'][0][0]}{game['dealer'][0][1]} ??"
...
async def ochko_start(query: CallbackQuery, state: FSMContext):
    ...
    if pv == 21:
        ...
        await query.message.edit_text(f"{ochko_text(game, True)}\n\n{msg}\nВыплата: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
        return await query.answer()
...
async def ochko_stand(query: CallbackQuery):
    ...
    OCHKO_GAMES.pop(user_id)
    await query.message.edit_text(f"{ochko_text(game, True)}\n\nРезультат: {'Победа' if payout > game['bet'] else ('Ничья' if payout == game['bet'] else 'Поражение')}\nВыплата: {fmt_money(payout)}\nБаланс: {fmt_money(bal)}")
    await query.answer()
...
async def checks_command(message: Message):
    await message.answer("🧾 Чеки", reply_markup=checks_kb())
...
async def checks_create_count(message: Message, state: FSMContext):
    ...
    if count < 1 or count > 100:
        return await message.answer("1-100")
    data = await state.get_data()
    amount = data["amount"]
...
async def bank_command(message: Message):
    ...
    conn.close()
    await message.answer(f"🏦 Банк\nБаланс: {fmt_money(float(user['coins']))}\nАктивных депозитов: {active['c']}\nСумма: {fmt_money(active['s'])}", reply_markup=bank_kb())
...
async def bank_amount(message: Message, state: FSMContext):
    try:
        ...
        return await message.answer("Введите сумму")
    if amount < 100:
        return await message.answer("Минимум 100")
    await state.update_data(amount=amount)
...
async def bank_list_cb(query: CallbackQuery):
    ...
    for r in rows:
        ...
        status = "✅ готов" if unlock <= now else f"⏳ {fmt_left(unlock-now)}"
...
async def bank_withdraw_cb(query: CallbackQuery):
    ...
    await query.answer()# ==================== АДМИНСКИЕ КОМАНДЫ ====================
...
async def addpromo_command(message: Message):
    ...
    if len(parts) != 4:
        return await message.answer("Формат: /addpromo CODE REWARD ACTIVATIONS")
...
async def admin_give(message: Message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        return await message.answer("Формат: выдасть @user сумма или ответ на сообщение")
    target = message.reply_to_message.from_user if message.reply_to_message else None
    if not target:
        try:
            target_id = int(parts[1])
...
async def admin_set_chance(message: Message):
    ...
    try:
        ...
        return await message.answer("Шанс 0-100")
    conn = get_db()
    conn.execute("INSERT OR REPLACE INTO game_settings (game_name, win_chance) VALUES (?, ?)", (game, chance))
    conn.commit()
    conn.close()
    await message.answer(f"✅ Для игры {game} установлен шанс {chance}%")
...
async def admin_stats(message: Message):
    ...
    bets = conn.execute("SELECT COUNT(*) as c, SUM(payout) as p FROM bets").fetchone()
    sys = conn.execute("SELECT coins FROM system_balance WHERE id=1").fetchone()
    conn.close()
    await message.answer(f"📊 Статистика\nПользователей: {users}\nСтавок: {bets['c']}\nВыплачено: {fmt_money(bets['p'] or 0)}\nКомиссий собрано: {fmt_money(sys['coins'] if sys else 0)}")
