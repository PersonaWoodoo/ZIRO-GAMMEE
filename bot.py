...
async def roulette_game(message: Message):
    ...
    ok, bal = settle_instant_bet(message.from_user.id, bet, payout, "roulette", outcome)
    if not ok:
...
async def crash_game(message: Message):
    ...
    ok, bal = settle_instant_bet(message.from_user.id, bet, payout, "crash", f"crash={rolled}")
    if not ok:
        return await message.answer(f"❌ Недостаточно средств")
...
async def cube_game(message: Message):
    ...
    if not ok:
        return await message.answer(f"❌ Недостаточно средств")
    await message.answer(f"🎲 <b>Кубик</b>\n🎲 Выпало: {rolled}\n{'✅ Победа! x5.8' if win else '❌ Поражение'}\n💰 Выплата: {fmt_money(payout)}\n💎 Баланс: {fmt_money(bal)}")
...
async def dice_game(message: Message):
    ...
    payout = bet * mult if win else 0
    ok, bal = settle_instant_bet(message.from_user.id, bet, payout, "dice", f"{d1}+{d2}={total}")
    if not ok:
        return await message.answer(f"❌ Недостаточно средств")
...
async def football_game(message: Message):
    ...
    bal = finalize_reserved_bet(message.from_user.id, bet, payout, "football", f"value={value}")
    await message.answer(f"⚽ <b>Футбол</b>\n🏆 Результат: {result_title}\n{'✅ Победа! x1.85' if win else '❌ Поражение'}\n💰 Выплата: {fmt_money(payout)}\n💎 Баланс: {fmt_money(bal)}")
...
async def basket_game(message: Message):
    ...
    payout = bet * 1.85 if win else 0
    bal = finalize_reserved_bet(message.from_user.id, bet, payout, "basket", f"value={value}")
...
def diamond_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔹 1", callback_data="diamond:pick:1"), InlineKeyboardButton(text="🔹 2", callback_data="diamond:pick:2"), InlineKeyboardButton(text="🔹 3", callback_data="diamond:pick:3"), InlineKeyboardButton(text="🔹 4", callback_data="diamond:pick:4"),# ----------------------------- DIAMONDS (ПРОДОЛЖЕНИЕ) -----------------------------
...
async def ochko_start_confirm(query: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if not ok:
        await query.message.edit_text(f"❌ Недостаточно средств для игры")
        await query.answer()
        return
...
async def menu_ref_cb(query: CallbackQuery):
    ...
    conn.close()
    await query.message.edit_text(
        f"👥 <b>Реферальная программа</b>\n\n🔗 Ваша ссылка:\n<code>{link}</code>\n\n📊 Приглашено: {count}\n🎁 За друга: +{fmt_money(REFERRER_BONUS)} вам, +{fmt_money(REFERRED_BONUS)} другу",
        reply_markup=main_menu_kb()
    )
    await query.answer()
...
async def menu_add_chat_cb(query: CallbackQuery):
    ...
    await query.message.answer(
        f"🤖 <b>Добавить бота в чат</b>\n\nНажмите на кнопку ниже:\n\n<a href='{link}'>➕ Добавить {BOT_USERNAME} в чат</a>",
